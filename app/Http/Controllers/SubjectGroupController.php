...
class SubjectGroupController extends Controller
{
    ...
    {
        $subjectGroup = SubjectGroup::select('course_id', 'name', 'semester_id')->distinct()->get();
    ...
    public function save_subject_group(Request $request)
    {
        ...
        $check = SubjectGroup::whereCourseId($course_id)->whereSemesterId($semester_id)->first();
        if ($check) {
...
//        return $name;
        ...
        if ($tempSem) {
            foreach ($tempSem as $d_sem) {
                $data = SubjectGroup::whereCourseId($d_sem->course_id)->whereSemesterId($semester_id)->whereSubjectId($d_sem->subject_id)->first();
                $data->delete();
            }
        }
    ...
    public function delete_subject_group($course_id,$semester_id)
    {
        $data = SubjectGroup::whereCourseId($course_id)->whereSemesterId($semester_id)->get();
